...
Tri-path HA connectivity is available on FAS2820 HA pairs. Tri-path HA connectivity is three paths for each controller to a shelf/stack...occurs because of the internal shelf connections, which give an HA pair multipath HA connectivity plus the cabling of the external storage ports (0a and 0b1) across the two controllers, if there are no external shelves, or to external shelves.
...
The FAS2820 SAS ports are 0a and 0b1: 

* The 0b1 port behaves the same as 0b ports on other platforms with internal storage. Only the label has changed.
