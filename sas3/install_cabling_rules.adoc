...
*  The controller's internal connections of port 0c to the internal shelf gives an HA pair multipath HA connectivity.

* The cabling of the controller's external storage ports 0a and 0b1 give an HA pair tri-path HA connectivity.
+
Ports 0a and 0b1 are cabled across the two controllers when there are no external shelves, or they are cabled to external shelves. 
...
* The 0a port is the internal HBA port (like other platforms with internal storage).
* The 0b1 port is the internal storage port (like the 0b ports on other platforms with internal storage).
* The 0b2 port is from the internal storage, but is not used. It is disabled. If a cable is connected to it, an error message is generated.
