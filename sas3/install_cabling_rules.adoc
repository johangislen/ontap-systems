 ...
 ** Platforms with internal storage can be cabled as single-path HA configurations (from port 0a) to support connectivity to an external SAS tape backup device.
 ...
 For FAS2800 HA pairs, although the cabling to the external shelves is single-path HA, because of the internal storage connectivity, the HA pair configuration is multipath HA. Information about the FAS2800 internal storage connectivity can be found in the <<Tri-path HA connectivity>> section.
