...
[NOTE]
====
For FAS2800 HA pairs, although the cabling to external shelves is single-path HA, because of each controller's internal connection of port 0b to its local expander (IOM12G) and port 0c to its partner’s expander, the HA pair configuration is multipath HA.
====
...
* Each controller’s internal connection of port 0b to its local expander (IOM12G) and port 0c to its partner’s expander gives an HA pair multipath HA connectivity.
