...
NOTE: This procedure applies only to systems running ONTAP in an HA pair.

CAUTION: Disk reassignment is only needed when replacing the NVRAM module. 
...
. If storage encryption is enabled, you must ********
...
[.lead]
Use the appropriate procedure if you have storage encryption enabled.
...
. Enter the onboard key manager passphrase for the cluster at the prompt:
Need example here******
. Enter the Backup key data 
+
NOTE: The node returns to the boot menu after the passphrase information is completed.
+
...
Need example here*****
+
NOTE: The node returns to the boot menu after the management certificate information is completed.
+
. Select option 1, `Normal Boot`
...
====
. Reset the MSID for the disks:
.. If you are using onboard storage encryption, follow the procedure in https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-onboard-task.html[Assign a data authentication key to a FIPS drive or SED (onboard key management)^]
.. I you are using external keymanager, follow the procedure in https://docs.netapp.com/us-en/ontap/encryption-at-rest/assign-authentication-keys-seds-external-task.html[Assign a data authentication key to a FIPS drive or SED (external key management)^]
