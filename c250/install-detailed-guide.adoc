...
NOTE: link:https://hwu.netapp.com[NetApp Hardware Universe^] slot priority for host network cards (FC or 25GbE) is slot 2. However, if you have both cards, the FC card goes in slot 2 and the 25GbE card goes in slot 1 (as shown below). If you have an external shelf, the storage card goes in slot 1, the only supported slot for shelves.
