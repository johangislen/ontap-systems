---
permalink:  legal-notices.html
sidebar: sidebar
keywords:
summary:
---
= Legal notices
:icons: font
:imagesdir: ../media/

// Include the common notices
include::https://raw.githubusercontent.com/NetAppDocs/common/main/_include/common-legal-notices.adoc[]

// Include the open source notice intro
include::https://raw.githubusercontent.com/NetAppDocs/common/main/_include/open-source-notice-intro.adoc[]

// Add required link(s)
https://library.netapp.com/ecm/ecm_download_file/ECMM1280364[Notice for disk shelves^]

//Added new section 28jul2022

include::https://raw.githubusercontent.com/NetAppDocs/common/main/_include/safety-regulatory-notice.adoc[]

https://library.netapp.com/ecm/ecm_download_file/ECMP12475945[^]
