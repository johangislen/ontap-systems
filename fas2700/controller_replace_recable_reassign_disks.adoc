...
HA pair
a|
<<Option 1: Verifying the system ID change on an HA system>>
a|
Stand-alone
a|
<<Option 2: Manually reassigning the system ID on a stand-alone system in ONTAP>>
a|
Two-node MetroCluster configuration
a|
<<Option 3: Manually reassigning the system ID on systems in a two-node MetroCluster configuration>>
|===
