...
summary: "Move the power supplies, hard drives, and controller module or modules from the impaired chassis to the replacement chassis, and swap out the impaired chassis from the equipment rack or system cabinet with the replacement chassis of the same model as the impaired chassis."
---
= Move and replace hardware - FAS2800
:icons: font
:imagesdir: ../media/
...
. Repeat these steps for the second controller module in the chassis.
...
. Remove the drives:
 .. Press the release button on the opposite side of the LEDs.
...
. Align the drive from the impaired chassis with the same bay opening in the replacement chassis.
...
. Firmly push the drive the rest of the way into the chassis, and then lock the cam handle by pushing it against the drive holder.
+
...
Install the controller module and any other components into the replacement chassis, boot it tto Maintenance mode.
...
. Repeat the preceding steps for the second controller in the replacement chassis.
...
NOTE: Do not use excessive force when sliding the controller module into the chassis to avoid damaging the connectors.
 ...
 .. Repeat the preceding steps for the second controller module in the replacement chassis.
