...
== Step 3:  Remove the adapter card  
...
== Step: 4 Install the mezzanine card in the new controller
. Reinstall the mezzanine card by aligning the socket on the motherboard with the socket on the mezzanine card, and then gently seat the card in the socket.
...
If your AFF A220 or FAS2800 system has a caching module, you need to move the caching module from the old controller module to the replacement controller module. The caching module is referred to as the "`M.2 PCIe card`" on the controller module label.
