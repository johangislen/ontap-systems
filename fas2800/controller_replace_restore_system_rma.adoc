...
Restore your system to full operation by restoring the NetApp Storage Encryption configuration (if necessary), and installing licenses for the replacement controller, and returning the failed part to NetApp, as described in the RMA instructions shipped with the kit.
