...
[.lead]
To replace the mezzanine card, you must disconnect the cables and any SFP and QSFP modules from the card, replace the failed mezzanine card, and then recable the cards.
...
+
image::../media/drw_2850_replace_HIC_IEOPS-700.svg.svg[]
...
.. Align the socket on the mezzanine card plug with the socket on the motherboard, and then gently seat the card squarely into the socket.
